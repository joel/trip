...
RSpec.describe Checklists::Tools::Delete do
  describe ".call" do
    it "deletes a checklist on a writable trip" do
      ...
      expect(Checklists::Checklist.exists?(checklist.id)).to be(false)
    end
    ...
    it "rejects deletion on a non-writable trip" do
      ...
      expect(Checklists::Checklist.exists?(checklist.id)).to be(true)
    end
